...
from EnvUAV_TD3.env import YawControlEnv_TD3
...
def test_fixed_traj(params, length, shape_type):
    ...
    # np.save("pos_PID+RS_Ellipse.npy", pos)
    # np.save("ang_PID+RS_Ellipse.npy", ang)
...
def test_fixed_traj_300_4700(params1, params2, length, shape_type):
    ...
    env.new_PD_params(PD_params=params1)
    for i in range(300):
        target = targets[i, :]
        env.step(target)
        pos.append(env.current_pos.tolist())
    ...
    env.new_PD_params(PD_params=params2)
