...
def _load_yaw_env_class(envuav_dir, tag):
    parent_pkg = f"_bo_{tag}"
    env_pkg = f"{parent_pkg}.EnvUAV"
    env_mod_name = f"{env_pkg}.env"
    if env_mod_name in sys.modules:
        return sys.modules[env_mod_name].YawControlEnv

    project_dir = os.path.dirname(envuav_dir)
    if parent_pkg not in sys.modules:
        parent_mod = types.ModuleType(parent_pkg)
        parent_mod.__path__ = [project_dir]
        sys.modules[parent_pkg] = parent_mod

    init_file = os.path.join(envuav_dir, "__init__.py")
    pkg_spec = importlib.util.spec_from_file_location(
        env_pkg, init_file, submodule_search_locations=[envuav_dir]
    )
    pkg_mod = importlib.util.module_from_spec(pkg_spec)
    sys.modules[env_pkg] = pkg_mod
    pkg_spec.loader.exec_module(pkg_mod)

    env_file = os.path.join(envuav_dir, "env.py")
    env_spec = importlib.util.spec_from_file_location(env_mod_name, env_file)
    env_mod = importlib.util.module_from_spec(env_spec)
    sys.modules[env_mod_name] = env_mod
    env_spec.loader.exec_module(env_mod)
    return env_mod.YawControlEnv


_ROOT_DIR = os.path.dirname(os.path.abspath(__file__))
YawControlEnv = _load_yaw_env_class(
    os.path.join(_ROOT_DIR, "PID", "EnvUAV"), "pid_env"
)
YawControlEnv_TD3 = _load_yaw_env_class(
    os.path.join(_ROOT_DIR, "TD3", "4_Pos", "EnvUAV"), "td3_4_pos_env"
)
...
def _plot_tracking_series(pos, ang, targets, pos_error, ang_error):
    index = np.arange(len(pos)) * 0.01
    zeros = np.zeros_like(index)
    px, py, pz = pos[:, 0], pos[:, 1], pos[:, 2]
    roll, pitch, yaw = ang[:, 0], ang[:, 1], ang[:, 2]
...
def _animate_tracking(pos, targets):
    length = len(pos)
    animation_Trajectory(
        t_all=np.arange(length) * 0.01,
        dt=0.01,
        x_list=pos[:, 0],
        y_list=pos[:, 1],
        z_list=pos[:, 2],
...
def _visualize_tracking(pos, ang, targets):
    pos_error, ang_error = _compute_tracking_errors(pos, ang, targets)
    _print_tracking_errors(pos_error, ang_error)
    _plot_tracking_3d(pos, targets)
    _plot_tracking_series(pos, ang, targets, pos_error, ang_error)
    _animate_tracking(pos, targets)
    return pos_error, ang_error
...
def test_fixed_traj(params, length, shape_type):
    ...
    try:
        pos, ang = _collect_rollout(env, targets, "current_ori")
    finally:
        env.close()

    printPID(env)
    return _visualize_tracking(pos, ang, targets)


# 测试 PID 参数 (随机轨迹)
def test_random_traj(params, length, targets):
    targets = np.asarray(targets)[:length]
    env = YawControlEnv()
    env.reset(base_pos=np.array([0, 0, 0]), base_ori=np.array([0, 0, 0]))
    env.set_pid_params(PID_params=params)
    print("PID ", "Random trajectory")
    ...
    printPID(env)
    _visualize_tracking(pos, ang, targets)


# 测试 TD3 算法 (指定轨迹)
def test_fixed_traj_TD3(length, shape_type):
    env = YawControlEnv_TD3()
    env.reset(base_pos=np.array([0, 0, 0]), base_ang=np.array([0, 0, 0]))
    targets, name = generate_target_trajectory(shape_type, length)
    print("TD3 ", name)

    try:
        pos, ang = _collect_rollout(env, targets, "current_ang")
    finally:
        env.close()

    np.save("pos_TD3_Spiral.npy", pos)
    np.save("ang_TD3_Spiral.npy", ang)
    _visualize_tracking(pos, ang, targets)


# 测试 TD3 算法 (随机轨迹)
def test_random_traj_TD3(length, targets):
    targets = np.asarray(targets)[:length]
    env = YawControlEnv_TD3()
    env.reset(base_pos=np.array([0, 0, 0]), base_ang=np.array([0, 0, 0]))
    print("TD3 ", "Random trajectory")
...
def test_fixed_traj_300_4700(params1, params2, length, shape_type):
    env = YawControlEnv()
    env.reset(base_pos=np.array([0, 0, 0]), base_ori=np.array([0, 0, 0]))
    targets, name = generate_target_trajectory(shape_type, length)
    print("PID ", name)
    ...
    pos = np.asarray(pos)
    ang = np.asarray(ang)
    return _visualize_tracking(pos, ang, targets[:length])
